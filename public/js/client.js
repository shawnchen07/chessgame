var Client = (function(window) {
  ...
  var clickVariable = 'click blink doubleblink';
  ...
  var attachDOMEventHandlers = function() {
    ...
    if (playerColor === 'white') {
      container.on(clickVariable, '.white.pawn', function(ev) {
        if (gameState.activePlayer && gameState.activePlayer.color === playerColor) {
          highlightValidMoves('wP', ev.target);
        }
      });
      container.on(clickVariable, '.white.rook', function(ev) {
        if (gameState.activePlayer && gameState.activePlayer.color === playerColor) {
          highlightValidMoves('wR', ev.target);
        }
      });
      container.on(clickVariable, '.white.knight', function(ev) {
        if (gameState.activePlayer && gameState.activePlayer.color === playerColor) {
          highlightValidMoves('wN', ev.target);
        }
      });
      container.on(clickVariable, '.white.bishop', function(ev) {
        if (gameState.activePlayer && gameState.activePlayer.color === playerColor) {
          highlightValidMoves('wB', ev.target);
        }
      });
      container.on(clickVariable, '.white.queen', function(ev) {
        if (gameState.activePlayer && gameState.activePlayer.color === playerColor) {
          highlightValidMoves('wQ', ev.target);
        }
      });
      container.on(clickVariable, '.white.king', function(ev) {
        if (gameState.activePlayer && gameState.activePlayer.color === playerColor) {
    ...
    if (playerColor === 'black') {
      container.on(clickVariable, '.black.pawn',   function(ev) {
        if (gameState.activePlayer && gameState.activePlayer.color === playerColor) {
          highlightValidMoves('bP', ev.target);
        }
      });
      container.on(clickVariable, '.black.rook',   function(ev) {
        if (gameState.activePlayer && gameState.activePlayer.color === playerColor) {
          highlightValidMoves('bR', ev.target);
        }
      });
      container.on(clickVariable, '.black.knight', function(ev) {
        if (gameState.activePlayer && gameState.activePlayer.color === playerColor) {
          highlightValidMoves('bN', ev.target);
        }
      });
      container.on(clickVariable, '.black.bishop', function(ev) {
        if (gameState.activePlayer && gameState.activePlayer.color === playerColor) {
          highlightValidMoves('bB', ev.target);
        }
      });
      container.on(clickVariable, '.black.queen',  function(ev) {
        if (gameState.activePlayer && gameState.activePlayer.color === playerColor) {
          highlightValidMoves('bQ', ev.target);
        }
      });
      container.on(clickVariable, '.black.king',   function(ev) {
        if (gameState.activePlayer && gameState.activePlayer.color === playerColor) {
    ...
    container.on(clickVariable, '.empty', function(ev) {
      clearHighlights();
    });

    // Perform a regular move
    container.on(clickVariable, '.valid-move', function(ev) {
    ...
    container.on(clickVariable, '.valid-capture', function(ev) {
    ...
    container.on(clickVariable, '.valid-en-passant-capture', function(ev) {
      var m = capture(ev.target);
      messages.empty();
    ...
    // Perform a castle
    container.on(clickVariable, '.valid-castle', function(ev) {
  ...
  var showPawnPromotionPrompt = function(callback) {
    ...
    pawnPromotionPrompt.one(clickVariable, 'button', function(ev) {
  ...
  var showForfeitPrompt = function(callback) {
    ...
    forfeitPrompt.one(clickVariable, '#cancel-forfeit', function(ev) {
      callback(false);
      forfeitPrompt.modal('hide');
    });

    // Temporarily attach click handler for the Confirm button, note the use of .one()
    forfeitPrompt.one(clickVariable, '#confirm-forfeit', function(ev) {
      callback(true);
      forfeitPrompt.modal('hide');
    });
